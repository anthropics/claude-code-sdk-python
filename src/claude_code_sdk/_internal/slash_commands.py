...
class SlashCommand(ABC):
    ...
    @abstractmethod
    async def execute(
        self, args: list[str], client: "StatefulCLIClient"
    ) -> CommandResult:
        """Execute the command."""
        pass
...
class ExitCommand(SlashCommand):
    ...
    async def execute(
        self, _args: list[str], client: "StatefulCLIClient"
    ) -> CommandResult:
        """End the current session."""
        await client.end_session()

        return CommandResult(
            command="exit", success=True, result="Session ended", message="Goodbye!"
        )
...
class HistoryCommand(SlashCommand):
    ...
    async def execute(
        ...
        return CommandResult(
            command="history", success=False, result=None, message="No active session"
        )
    ...
    async def execute(
        ...
        if client._persistence:
            ...
            for session in sessions:
                active_marker = (
                    " (ACTIVE)" if session.session_id == client.get_session_id() else ""
                )
                start_time = session.start_time.strftime("%Y-%m-%d %H:%M")
                sessions_text += f"• {session.session_id}{active_marker}\n"
                sessions_text += (
                    f"  Started: {start_time}, Messages: {session.message_count}\n"
                )
...
class InfoCommand(SlashCommand):
    ...
    async def execute(
        ...
        return CommandResult(
            command="info", success=False, result=None, message="No active session"
        )
...
class SearchCommand(SlashCommand):
    ...
    async def execute(
        ...
        if client._transport and client._transport.get_session_data():
            ...
            results_text = (
                f"Found {len(matching_messages)} messages containing '{query}':\n\n"
            )
...
class SlashCommandRouter:
    ...
    async def execute_command(
        self, text: str, client: "StatefulCLIClient"
    ) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                command=text, success=False, result=None, message=f"Command failed: {e}"
            )
