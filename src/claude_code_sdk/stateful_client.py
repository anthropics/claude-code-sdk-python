...
from ._internal.session_persistence import (
    ConversationMessage,
    SessionData,
    SessionPersistence,
    SessionSummary,
)
from ._internal.slash_commands import CommandResult, SlashCommand, SlashCommandRouter
...
class StatefulCLIClient:
    ...
    def is_session_active(self) -> bool:
        ...
        return (
            ...
            and self._transport.is_session_active()
        )
    ...
    async def send_message(
        self, message: str, stream_response: bool = True
        ...
        if (
            self._enable_slash_commands
            and self._command_router
            and self._command_router.is_command(message)
        ):
        ...
        try:
            if stream_response:
                return self._stream_responses(message)
            else:
                # Get simple JSON response using the new unified API
                response_result = await self._transport.send_user_message(
                    message, stream=False
                )
                # Type guard: ensure we get dict, not AsyncIterator
                if not isinstance(response_result, dict):
                    raise CLIConnectionError(
                        "Expected dict response but got AsyncIterator"
                    )
                response_data = response_result

                # Auto-save after response if enabled
                if self._auto_save:
                ...
                return ResponseMessage(
                    ...
                    timestamp=datetime.now(),
                    message_type="text",
                    session_id=response_data.get("session_id", self._session_id),
    ...
    async def _stream_responses(self, message: str) -> AsyncIterator[ResponseMessage]:
        """Stream responses from the transport."""
        if not self._transport:
            return

        try:
            message_stream = await self._transport.send_user_message(
                message, stream=True
            )
            # Type guard: ensure we get AsyncIterator, not dict
            if isinstance(message_stream, dict):
                raise CLIConnectionError("Expected AsyncIterator but got dict")

            async for cli_message in message_stream:
                # Convert CLI message to ResponseMessage
                if cli_message.get("type") == "assistant":
                    # Assistant message with potential tool usage
                    message_data = cli_message.get("message", {})
                    ...
                    response = ResponseMessage(
                        ...
                        session_id=cli_message.get("session_id", self._session_id),
                        usage=TokenUsage(**message_data["usage"])
                        if message_data.get("usage")
                        else None,
                    )
                    yield response

                elif cli_message.get("type") == "result":
                    # Final result message
                    response = ResponseMessage(
                        content=cli_message.get("result", ""),
                        role="assistant",
                        timestamp=datetime.now(),
                        message_type="result",
                        session_id=cli_message.get("session_id", self._session_id),
                        usage=TokenUsage(**cli_message["usage"])
                        if cli_message.get("usage")
                        else None,
                    )

                    # Auto-save after final response if enabled
                    if self._auto_save:
                        await self._transport.save_session()

                    yield response

                elif cli_message.get("type") == "system":
                    # System initialization message - can be ignored or logged
                    logger.debug(f"System message: {cli_message}")

                elif cli_message.get("type") == "user":
                    # Tool result message - can be ignored for client purposes
                    logger.debug(f"Tool result: {cli_message}")
    ...
    async def register_command(
        self, command: str, handler: SlashCommand, description: str = ""
    ) -> None:
        """
    ...
    async def set_working_directory(self, path: str | Path) -> None:
        ...
        if session_data:
            ...
            session_data.metadata["working_directory_updated"] = (
